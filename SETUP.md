...
- **관리자 판별**: `src/admin.js`의 `ADMIN_CODE`를 아는 사람만 관리자가 돼요. `사이트주소/#admin-비밀코드` 로 한 번 접속하면 **그 브라우저만** 관리자로 등록됩니다 (닉네임 칩에 🛡️ 표시, 주소창의 코드는 자동으로 지워짐). 해제하려면 `사이트주소/#admin-off`.
  - IP 방식은 같은 와이파이(회사망) 사용자 전원이 관리자가 되는 문제가 있어서 쓰지 않아요.
- **버블 터뜨리기**: 관리자에게만 우상단에 🧨 스위치가 보여요. 켜면 버블에 빨간 테두리가 생기고, 이 상태에서 버블을 누르면 확인 없이 바로 터져요 (구매 완료 처리). 터진 버블은 **구매완료 히스토리**(우상단 🏆 아이콘)에 이름/사진만 남아요. 다 터뜨렸으면 스위치를 꺼서 일반 모드로 돌아오세요.
- **히스토리 말소**: 관리자는 히스토리 항목의 ✕ 버튼으로 기록도 지울 수 있어요.
- **코드가 새면**: `src/admin.js`의 `ADMIN_CODE`를 새 값으로 바꿔 배포하면 기존 관리자 브라우저가 전부 무효화돼요. 새 코드 링크로 다시 등록하면 됩니다.

> ⚠️ 솔직한 참고: Firestore 규칙은 이 비밀 코드를 검증할 수 없어서, 삭제 권한은 규칙 차원에서는 열려 있고 **화면(UI)에서만 관리자에게 노출**돼요. 코드도 배포된 소스에 들어 있어서 개발자 도구를 파헤치면 찾을 수는 있는 구조라, 사내용 익명 서비스라는 전제에서의 가벼운 잠금장치입니다.
...
**Q. 쿠팡 링크가 꼭 있어야 하나요?**
아니요. 쿠팡 링크는 선택입니다. 이름만 적어도 건의가 올라가고, 링크를 넣으면 상세 화면에 "보러 가기" 버튼이 생겨요. 쿠팡이 아닌 주소도 됩니다.

**Q. 상품 사진은 어떻게 넣나요?**
셋 중 아무거나 편한 방법으로요.
- **업로드** : 사진 영역을 탭해서 파일 고르기 (휴대폰이면 카메라·앨범)
- **드래그앤드랍** : 건의 창이 열린 상태에서 사진 파일을 화면에 끌어다 놓기
- **링크** : 이미지 주소를 복사해 붙여넣기 (사진을 복사했다면 그대로 ⌘V / Ctrl+V 해도 됩니다)

올린 사진은 긴 변 900px, 약 320KB 이하로 자동 압축돼 문서 안에 함께 저장되므로 Firebase Storage를 따로 켤 필요가 없어요.
...
**Q. 흘러가는 문구는 어떻게 바꾸나요?**
두 가지가 따로 놉니다.
- **맨 위 띠(고정 공지)** : `src/ui.js` 맨 위 `NOTICE` 한 줄을 고치면 됩니다. 빈 문자열로 두면 띠가 사라져요.
- **버블 뒤 배경 글씨** : 우상단 📢 아이콘으로 누구나 띄웁니다 (60자). 최근 12개가 버블맵 뒤에서 제각각 다른 높이·속도·크기로 흘러가고, 관리자에게는 목록에 ✕가 보여 내릴 수 있어요.
